...
import (
	...
	"github.com/appcelerator/amp/docker/docker/pkg/progress"
	"github.com/appcelerator/amp/docker/docker/pkg/streamformatter"
...
// nolint: gocyclo
func ServiceProgress(ctx context.Context, client docker.APIClient, serviceID string, progressWriter io.Writer) error {
		...
		if converged {
			...
			if wait >= 0 {
				progressOut.WriteProgress(progress.Progress{
					...
					ID:     "verify",
					Action: fmt.Sprintf("Waiting %d seconds to verify that service %s tasks are stable ...", wait/time.Second+1, service.Spec.Name),
				})
			}
		} else {
			if !convergedAt.IsZero() {
...
func getActiveNodes(ctx context.Context, client docker.APIClient) (map[string]struct{}, error) {
