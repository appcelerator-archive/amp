...
func (s *AMPInfraManager) Monitor(stack *ampStack) (*[]*servergrpc.TypedOutput, error) {
	...
	for name, serv := range stack.serviceMap {
		if serv.user {
			if name != "cluster-server" && name != "cluster-agent" {
	...
	listName := []string{}
	if _, ok := stack.serviceMap["cluster-server"]; ok {
		listName = append(listName, "cluster-server")
	}
	if _, ok := stack.serviceMap["cluster-agent"]; ok {
		listName = append(listName, "cluster-agent")
	}
	...
	for _, name := range listName {
		...
		if serv.status == "running" {
			if serv.user && serv.name != "cluster-server" && serv.name != "cluster-agent" { //TODO remove specific name usage need
