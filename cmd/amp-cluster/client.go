...
func (g *clusterClient) initConnection() error {
	...
	if g.isLocalhostServer() {
		...
		if !g.clusterLoader.isServiceRunning("cluster-server") {
...
func (g *clusterClient) connectServer() error {
	conn, err := grpc.Dial(g.configuration.ClusterServerAddress,
		grpc.WithInsecure(),
		grpc.WithBlock(),
...
func (g *clusterClient) startServerReader() error {
	...
	g.stream = stream
	g.printfc(colDebug, "Connected to cluster-server, waiting server ack\n")
...
func (g *clusterClient) isLocalhostServer() bool {
	if strings.HasPrefix(g.configuration.ClusterServerAddress, "127.0.0.1") || strings.HasPrefix(g.configuration.ClusterServerAddress, "localhost") {
		return true
	}
	return false
}
