...
func (s *clusterStack) init(local bool, ampTag string) {
	...
	s.addService("cluster-server", "amp", 1,
		&swarm.ServiceSpec{
	...
	s.addService("cluster-agent", "amp", 0,
		&swarm.ServiceSpec{
			Annotations: swarm.Annotations{
				Labels: map[string]string{
				//"io.amp.role": "infrastructure",
