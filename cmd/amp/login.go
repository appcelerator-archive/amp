...
var ampLoginCmd = &cobra.Command{
	Use:   "login",
	Short: "Login via GitHub",
	Long:  `The login command creates a GitHub access token and stores it in your Config file to authenticate further commands.`,
...
func init() {
	RootCmd.AddCommand(ampLoginCmd)
}

// AmpLogin creates a github access token and store it in your Config file to authenticate further commands
func AmpLogin(a *client.AMP) error {
...
func getAmpPassword() (password string, err error) {
	color.Set(color.FgMagenta, color.Bold)
	defer color.Unset()
